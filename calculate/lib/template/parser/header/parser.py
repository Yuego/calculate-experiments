...
from calculate.lib.template.parser.condition.parser import ConditionParser
from calculate.lib.template.parser.rules import identifier, package_atom, quoted_string
...
class TemplateHeaderParser(ConditionParser):
    ...
    def get_syntax(self):
        ...
        if 'format' in self.parameters:
            import calculate.lib.template.formats as _f
            for importer, modname, ispkg in pkgutil.iter_modules(_f.__path__, _f.__name__ + '.'):
                self.parameters['format'].append(modname)
